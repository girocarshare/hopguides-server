		...
		if (!undefinedValue(value)) {
			if (isPrimitive(type)) {
				...
				if (!nullValue(value)) {
					...
					else obj[field.propertyKey] = value.map(e => deserialize(field.type, e));
				}
			} else {
...
export function serialize(obj: any, filter?: string): any {
	...
	for (const field of jsonFields.filter(f => f.serialize)) {
		...
		if ((!undefinedOrNullValue(value) || nullValue(value)) && isFilterMatch(filter, field)) {
...
function isFilterMatch(filter: string, jsonField: JsonPropertyMetaData): boolean {
