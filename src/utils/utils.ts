...
import type { INextFunction, IRequest, IResponse } from '../classes/interfaces';
...
export function generateVehicleIMEI(text?: string): string {
  const vehicleIMEI = `#${onlyChar(text)}${Math.floor(
...
const JWT_KEY = '73c0f68d35b4fc866d54';
var JWT_SECRET =
  'jj8axcJBhpQqZm08O5HxGxpSH9XLmQFhXYbPmt6wnG8B4Q92N98zSPmJrgtceOi';
...
class RoleAndIdChecker extends RoleChecker {
  ...
  check(id: any, role: string, req: IRequest): boolean {
    return (
      super.check(id, role, req) && (!this.idChecker || this.idChecker(id, req))
    );
  }
}
...
export function allowFor(
  ...
  return function (req: IRequest, res: IResponse, next: INextFunction): any {
    ...
    const authorized = rules.some((r) =>
      r.check(tokenData.id, tokenData.role, req),
    );
