...
function validatePrimitiveType(
  ...
  if (typeof value !== getNativeType(type)) {
    errors.push(
      `Property (${propertyKey}) with value (${value}) is not of type ${getNativeType(
        type,
      )}.`,
    );
  }
  ...
  if (errors.length === 0) {
    ...
    for (const validation of validations) {
      ...
      for (const validator of validation.validators) {
        ...
        if (errorMessage)
          errors.push(errorMessage.printOut(validation.propertyKey, value));
      }
    ...
    if (errors.length === 0) {
      ...
      for (const validationMethod of validationMethods) {
        const validationFunction =
          validationMethod.validationFunction.bind(target);
