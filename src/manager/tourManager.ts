...
export class TourManager {
  ...
  async generateQr(tourId: string, providerId: string): Promise<boolean> {
    ...
    await QRCode.toFile(
      'images/tourQrCodes/' +
        ...
        providerId.trim() +
        '.png',
      'http://localhost:3000/deeplink?url=https://www.youtube.com/watch?v=AYO-17BDVCw&list=RDAYO-17BDVCw&start_radio=1',
      {
        scale: 15,
        width: '1000px',
      },
  ...
  async getToursForReport(
    ...
    var tours: Tour[] = await this.tourRepository
      .getAll(filter, pagination)
      .catch(() => {
        throw new Error('Error getting Tours');
      });
