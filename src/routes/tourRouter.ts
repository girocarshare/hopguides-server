  ...
  init(): void {
    ...
    this.router.get(
      ...
      withErrorHandler(async (req: IRequest, res: IResponse) => {
        try {
          ...
          if (tour != null) {
            await this.tourManager.generateQr(
              req.params.tourId,
    ...
    this.router.post(
      ...
      withErrorHandler(async (req: IRequest, res: IResponse) => {
        ...
        await this.tourManager.updateTour(tour.id, deserialize(Tour, tour));
        ...
        try {
          for (var point of req.body.points) {
            ...
            if (poi != null) {
              ...
              return res
                .status(500)
                .send("Error point with that id doesn't exist");
            }
          }
    ...
    this.router.post(
      ...
      withErrorHandler(async (req: IRequest, res: IResponse) => {
        try {
          ...
          const tour: Tour = await this.tourManager.getTour(req.body.id);
          ...
          const tours: ToursReport[] =
            await this.tourManager.getToursForReport();
          return res.status(200).send(tours);
        } catch (err) {
          console.log(err.error);
        }
